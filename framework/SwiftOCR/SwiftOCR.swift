...
public class SwiftOCR {
        ...
        if let inputImage = image ?? self.image {
            ...
            for label in 0..<parentArray.count {
                ...
                if minMaxCorrect && correctFormat && notToTall && notToWide && notToShort && notToThin && notToSmall && positionIsOK{
                    let labelRect = CGRectMake(CGFloat(minX - mergeRadius), CGFloat(minY - mergeRadius), CGFloat(maxX - minX + 2*mergeRadius + 1), CGFloat(maxY - minY + 2*mergeRadius + 1))
            ...
            for rect in mergeLabelRects {
                let cropRect = rect.insetBy(dx: CGFloat(mergeRadius), dy: CGFloat(mergeRadius))
    ...
    internal func preprocessImageForOCR(image:UIImage?) -> UIImage? {
